 ...
 */
class ImageFileChooserPreview extends JComponent implements
	...
	/**
	 * This class requires a file chooser to register with so this class will be
	 * notified when a new file is selected in the browser.
	 * 
	 * @param parent -
	 *            that this preview window is used in.
	 */
	...
	public void loadImage() {
		...
		if (imageThumb.getIconHeight() < previewHeight
				&& imageThumb.getIconWidth() < previewWidth) {
			return;
		}
		int w = previewWidth;
		int h = previewHeight;
		...
		imageThumb = new ImageIcon(imageThumb.getImage().getScaledInstance(w,
				h, Image.SCALE_DEFAULT));
	}

	/**
	 * Callback (event handler) to indicate that a property of the JFileChooser
	 * has changed. If the selected file has changed cause a new thumbnail to
	 * load.
	 */
	 ...
	 * 
	 * @param g -
	 *            object to use when painting the component.
	 */
