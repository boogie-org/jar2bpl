...
public class AssignmentTranslation {
	...
	public static void translateAssignment(SootStmtSwitch ss, Value lhs, Value rhs, Unit statement) {
		...
		if (rhs instanceof InvokeExpr) {
			...
			InvokeTranslation.translateInvokeAssignment(ss, lhs, ivk, statement);
			return;
		}
		...
		if (rhs instanceof NewExpr) {
			...
			ss.addStatement(GlobalsCache.v().setArraySizeStatement(
					right, sizeexp));
			...
			translateAssignment(ss, SootPrelude.v().getStringSizeHeapVariable(),
	 ...
	 * @param right
	 */
	public static void translateAssignment(SootStmtSwitch ss, Expression left, Expression right) {
		ProgramFactory pf = GlobalsCache.v().getPf();
		if (left instanceof IdentifierExpression) {
			...
			Expression arraystore = pf.mkArrayStoreExpression(aae
